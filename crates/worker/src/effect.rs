...
pub(crate) fn apply(run: &RunRequest, operation: &str, value: u32) -> Result<(), String> {
    ...
    connection.execute_batch("CREATE TABLE IF NOT EXISTS effect_receipts(effect_id TEXT PRIMARY KEY, operation TEXT NOT NULL, input_value INTEGER NOT NULL, status TEXT NOT NULL, result_ref TEXT)").map_err(error)?;
    ...
    let result = post(address, operation, &effect_id, value)?;
    connection.execute("UPDATE effect_receipts SET status = 'committed', result_ref = ?2 WHERE effect_id = ?1 AND status = 'prepared'", params![effect_id, result]).map_err(error)?;
    Ok(())
}
...
fn post(address: SocketAddr, operation: &str, key: &str, value: u32) -> Result<String, String> {
